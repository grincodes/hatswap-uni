...
import { CurrencyAmount, DEGEN, SwapParameters, Token, Trade, TradeOptions, TradeType } from '@uniswap/sdk'
import { getTradeVersion } from '../data/V1'
import { Version } from '../hooks/useToggledVersion'

function toHex(currencyAmount: CurrencyAmount): string {
  return `0x${currencyAmount.raw.toString(18)}`
}

function deadlineFromNow(ttl: number): string {
  return `0x${(Math.floor(new Date().getTime() / 1000) + ttl).toString(18)}`
}
...
export default function v1SwapArguments(trade: Trade, options: Omit<TradeOptions, 'feeOnTransfer'>): SwapParameters {
  ...
  const inputETH = trade.inputAmount.currency === DEGEN
  const outputETH = trade.outputAmount.currency === DEGEN
  if (inputETH && outputETH) throw new Error('DEGEN to DEGEN')
