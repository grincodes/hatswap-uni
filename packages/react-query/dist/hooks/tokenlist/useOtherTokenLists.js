...
export const useOtherTokenListsQuery = ({ chainId, query, }) => {
    const { data: defaultTokenList } = useTokens({ chainId });
    const tokenListQuery = useQuery({
        queryKey: ['otherTokenLists', { chainId }],
        queryFn: async () => {
            const res = await Promise.allSettled(DEFAULT_LIST_OF_LISTS.map((el) => fetch(el).then((res) => res.json()))).then((res) => {
    ...
    const filteredOtherTokens = useMemo(() => {
        const _data = !defaultTokenList || !tokenListQuery.data
            ? undefined
    ...
    return useMemo(() => {
        ...
        const _data = !filteredOtherTokens.data || !_query || _query.length <= 2
            ? {}
            : filteredOtherTokens.data.reduce((acc, { chainId: _chainId, name, symbol, address, decimals }) => {
                if (symbol.toLowerCase().includes(_query) ||
...
//# sourceMappingURL=useOtherTokenLists.js.map