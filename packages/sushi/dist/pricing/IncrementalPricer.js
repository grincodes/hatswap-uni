...
class TokenInfo {
    ...
    totalSuccessors = 0;
    changedPoolIndex;
    constructor(token, direction, poolPrice, parent) {
        this.token = token;
...
class IncrementalPricer {
    ...
    _fullPricesRecalculation(pools, logging = false) {
        ...
        sortedBaseVerts.forEach(([baseVert, baseIndex], i) => {
            ...
            while (nextEdges.length > 0) {
                ...
                vTo.obj = new TokenInfo(vTo.token, direction, p, vFrom.obj);
    ...
    _addNewToken(pool, direction, priceFrom) {
        ...
        const tokenToInfo = new TokenInfo(tokenTo, direction, poolPrice, tokenFromInfo);
