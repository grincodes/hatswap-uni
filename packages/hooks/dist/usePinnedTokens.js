...
import { STARGATE_USDC, STARGATE_USDT, STARGATE_WBTC, STARGATE_WETH, } from 'sushi/config';
import { ARB, DAI, FRAX, GNO, MATIC, MIM, SUSHI, USDB, USDC, USDT, WBTC, WETH9, WNATIVE, WORMHOLE_USDC, WORMHOLE_WBTC, WORMHOLE_WETH, ZETA_ETH_BRIDGE_USDC, ZETA_ETH_BRIDGE_USDT, axlDAI, axlETH, axlUSDC, axlUSDT, axlWBTC, } from 'sushi/currency';
...
export const usePinnedTokens = () => {
    const [pinnedTokens, setPinnedTokens] = useLocalStorage('sushi.pinned-tokens', {});
    const addPinnedToken = useCallback((currencyId) => {
        const [chainId, address] = currencyId.split(':');
        setPinnedTokens((value) => {
            value[chainId] = Array.from(new Set([
                ...(value[chainId] || []),
    ...
    const removePinnedToken = useCallback((currencyId) => {
        const [chainId, address] = currencyId.split(':');
        setPinnedTokens((value) => {
            value[chainId] = Array.from(new Set(value[chainId].filter((token) => token !== `${chainId}:${getAddress(address)}`)));
            return value;
        });
    }, [setPinnedTokens]);
    const hasToken = useCallback((currency) => {
        if (typeof currency === 'string') {
            ...
            return pinnedTokens?.[chainId]?.includes(`${chainId}:${getAddress(address)}`);
        }
        return !!pinnedTokens?.[currency.chainId]?.includes(currency.id);
    }, [pinnedTokens]);
    const mutate = useCallback((type, currencyId) => {
        if (type === 'add')
            addPinnedToken(currencyId);
        if (type === 'remove')
            removePinnedToken(currencyId);
...
//# sourceMappingURL=usePinnedTokens.js.map